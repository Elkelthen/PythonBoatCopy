#!/usr/bin/env python
"""
# GrovePi Library for using the Grove - 6-Axis Accelerometer&Compass v2.0
# (http://www.seeedstudio.com/depot/Grove-6Axis-AccelerometerCompass-v20-p-2476.html)
#
...
class LSM303D:
    ...
    def get_accel(self):
        """
        :return: raw accelerometer values
        """
        raw_accel = [0, 0, 0]
    ...
    def get_real_accel(self):
        """
        :return: accelerometer values in g
        """
        real_accel = [0.0, 0.0, 0.0]
        accel = self.get_accel()
        for i in range(3):
            real_accel[i] = round(accel[i] / math.pow(2, 15) * self.ACCELE_SCALE, 3)
        return real_accel

    def get_mag(self):
        """
        :return: compass raw values
        """
        raw_mag = [0, 0, 0]
    ...
    def get_heading(self):
        """
        :return heading from the compass:
        """
        mag_value = self.get_mag()
        heading = 180 * math.atan2(mag_value[self.Y], mag_value[self.X]) / math.pi
    ...
    def get_tilt_heading(self):
        ...
        """
        mag_value = self.get_mag()
        accel_value = self.get_real_accel()

        x_val = self.X
        y_val = self.Y
        z_val = self.Z

        pitch = math.asin(-accel_value[x_val])

        print(accel_value[y_val],
              pitch,
              math.cos(pitch),
              accel_value[y_val] / math.cos(pitch),
              math.asin(accel_value[y_val] / math.cos(pitch)))

        roll = math.asin(accel_value[y_val] / math.cos(pitch))

        xh_val = mag_value[x_val] * math.cos(pitch) + mag_value[z_val] * math.sin(pitch)
        yh_val = mag_value[x_val] * math.sin(roll) * math.sin(pitch) + mag_value[y_val] * \
             math.cos(roll) - mag_value[z_val] * math.sin(roll) * math.cos(pitch)

        heading = 180 * math.atan2(yh_val, xh_val) / math.pi
...
if __name__ == "__main__":
    acc_mag = LSM303D()
    while True:
        print(acc_mag.get_real_accel())

        while True:
            if acc_mag.is_mag_ready():
                break
        print(acc_mag.get_heading())
